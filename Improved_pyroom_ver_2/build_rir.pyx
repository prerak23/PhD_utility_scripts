...
def fast_window_sinc_interpolater(double time_fp):
...
#The output we get from here is in time domain and does not consist of any frequency response hence we convolve it with specific octave band filters and then add everything together for each octave band to get the final RIR for this microphone.
