...
class CardioidFamily(Directivity):
    ...
    def plot_response(
        ...
        return ax
    def rotate_mic(self,azimuth,colatitude,degrees=False):

        coord = spher2cart(azimuth=azimuth, colatitude=colatitude, degrees=degrees)


        n_c = np.pi * 0.5 - (self.get_colatitude(degrees=False))
        n_a = -(self.get_azimuth(degrees=False))
        ...
        res = np.matmul(R_y,R_z)
        res = np.matmul(res, coord)
        print(res)

        fg = plt.figure(figsize=plt.figaspect(0.5))
        ax1 = fg.add_subplot(1, 3, 1, projection='3d')
        ax1.scatter(coord[0,:], coord[1,:], coord[2,:], c="red", s=100,label="Original Coords IS")
        ax1.scatter(res[0, :], res[1, :], res[2, :], c="blue", s=100,label="Rotated Coords IS")

        ax1.set_title("Figure of Eight Receiver , Mag|STFT|")
        ax1.legend()
        plt.show()

        r = np.sqrt(np.square(res[0, :]) + np.square(res[1, :]) + np.square(res[2,:]))
        colatitude=np.arccos((res[2,:] / r))
        azimuth=np.arctan2(res[1,:], res[0,:])
        ...
        '''
                for i in range(azimuth.shape[0]):
                    ...
                    R_z = np.array([[np.cos(n_a), -np.sin(n_a), 0], [np.sin(n_a), np.cos(n_a), 0], [0, 0, 1]])
                    res = np.matmul(R_y, R_z)
                    res = np.matmul(res, coord[:,i])

                    r = np.sqrt(np.square(res[0]) + np.square(res[1]) + np.square(res[2]))
                    colatitude_ = np.arccos((res[2] / r))
                    azimuth_ = np.arctan2(res[1],res[0])
                    azimuth_1.append(azimuth_)
                    colatitude_1.append(colatitude_1)
                    #print("To Verify Calculation",np.rad2deg(colatitude_),azimuth_)
                '''
