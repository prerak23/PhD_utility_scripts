...
class interpolate_sofa_files():
    ...
    def __init__(self,deg,source=False,id=2,freq_dependent=True,azimuth_mic=0,colatitude_mic=0,mic_type=0):
        ...
        #self.debug_mic_orientation(np.deg2rad(23),np.deg2rad(73))
    ...
    def plot(self,exp_original_grid,exp_target_grid,fibo_debug,freq_bin):
        ...
        ax1.set_title("Figure of Eight Receiver , Mag|STFT|")
        ...
        ax3 = fg.add_subplot(1,4,3,projection='3d')
        ...
        ax3.set_title("Interpolated Target Grid Based On Analytic SOFA Receiver")
        fg.colorbar(ax_2, shrink=0.5, aspect=5)

        ax3 = fg.add_subplot(1, 4, 4, projection='3d')
        ax_2 = ax3.scatter(x_src_fibo, y_src_fibo, z_src_fibo, c=fibo_debug, s=50)
        ax3.set_title("Fibo Grid Totaly Made Using Analytic Formula ")
        fg.colorbar(ax_2, shrink=0.5, aspect=5)
    ...
    def neareast_neighbour(self,azimuth,colatitude):
        ...
        dd, ii = self.nn_kd_tree_rotated_fibo_grid.query([longitude, latitude])
...
for i in degree:
    ...
    l=interpolate_sofa_files(i,source=False,id=4,freq_dependent=False,azimuth_mic=np.deg2rad(0),colatitude_mic=np.deg2rad(0)) #When changing the source keep check of the freq bin , and receiver, source type by their index.
    ...
    l.plot(exp_original_grid_freqbin, exp_whole_target_grid[:, freq_bin], l.freq_independent_gain_fibo[:, freq_bin],freq_bin)
